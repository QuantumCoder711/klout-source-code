...
const ThankYouMessage: React.FC = () => {
    ...
    const handleSubmit = () => {
        ...
        try {
            axios.post(`${imageBaseUrl}/api/thank-you-message`, dataObj, {
                headers: {
                ...
                .then(res => {
                    ...
                    if (res.status === 200) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Success',
                            text: 'Thank you message was sent successfully!',
                        }).then((result) => {
            ...
            <div className='flex gap-5 mt-10 text-black'>
                <div className='border border-zinc-400 w-4/6 rounded-xl h-fit'>
                    <div className="card-header p-3 border-b border-zinc-400 bg-zinc-200 rounded-t-xl">
                        <h6 className="m-0 font-bold text-klt_primary-500">Send Thank You message to Attendee for {currentEvent.title}</h6>
                    </div>
